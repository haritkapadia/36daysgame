...
public class World {
	...
	public void startFire(int x, int y) {
		startFire(new Point(x, y));
	}

	public void startFire(Point p) {
		BlockKey b = getBlock((int)p.getX(), (int)p.getY(), 1);
		if(b != null) {
			destroyBlock((int)p.getX(), (int)p.getY(), 1);
			entities.add(new Fire("fire" + entities.getSize(), this, (int)p.getX(), (int)p.getY()));
		}
	}
